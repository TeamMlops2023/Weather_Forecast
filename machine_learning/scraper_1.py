# Scrape Toutes les données des stations désirées pour les 14 derniers mois et les stocker dans une table
...
def liste_14_last_months():
    """
    Génère une liste des 14 derniers mois au format YYYYMM.

    Returns:
        list: Liste des 14 derniers mois au format YYYYMM.
    """
  
    dates = []
    # Boucle pour générer les 14 derniers mois
    for i in range(14):
        # Soustrayez i mois de la date actuelle
        month = 1+(datetime.now().month-i-1)%12
        year = datetime.now().year+(datetime.now().month-i-1)//12
    
        # Ajoutez la date au format YYYYMM
        formatted_date = f"{year}{month:02}"
    
        # Ajoutez la date à la liste
        dates.append(formatted_date)

    # Inversez la liste pour traiter les plus anciennes dates avant
    dates.reverse()
...
def scraping(dates, df_scrap):
    """
    Effectue le scraping des données météorologiques pour les dates spécifiées.

    Args:
        dates (list): Liste des dates au format 'YYYYMM'.
    ...
    for i in range(len(df_scrap)):
        ...
        for date in dates:
            # URL => https://reg.bom.gov.au/climate/dwo/ + YYYYMM + /html/ + IDCJDWXXXX + .YYYYMM.shtml
            url = "".join(['https://reg.bom.gov.au/climate/dwo/', date, '/html/', id_url, '.', date, '.shtml'])
    
            page = urlopen(url)
            soup = bs(page, "html.parser")
            lignes = soup.table.tbody.find_all('tr')

            for day, ligne in enumerate(lignes):
                data = [ville, f"{date}{day+1:02}", Latitude, Longitude]
                colspan = 1
                colspan_val = ""
                colspan_done = 0
    ...
    last_14_months_data = pd.concat(dfs_list, ignore_index=True)

    return last_14_months_data

def save_data_scrapees(last_14_months_data):
    """
    Sauvegarde les données extraites dans une table sql weather_scraped.

    Args:
        last_14_months_data (pd.DataFrame): Le DataFrame contenant les données extraites.
    ...
    # last_14_months_data.to_csv('data_scrapees_1.csv', index=False)
...
# Processus de scraping des données des 14 derniers mois dont le mois en cours
dates = liste_14_last_months()
df_scrap = recuperation_param_pour_scrap()
last_14_months_data = scraping(dates, df_scrap)
save_data_scrapees(last_14_months_data)
